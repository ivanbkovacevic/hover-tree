...
let kvadSize=30;
let kvadNumberOf=30;
let kvadCont= document.querySelector('.testsquare');
let kvadOriginalArr=[];
let kvadObj={};
...
for(let i=0; i<6;i++){
  for(let k=0; k<6; k++){
    ...
    kvadObj={position:kvadPos,height:`${kvadSize}px`,width:`${kvadSize}px` ,bcgImage:`url(${img})`,
    bcgPosition:`${kvadSize*k*-1}px ${kvadSize*i*-1}px`, zIndex:`99`,transition:`all 0.8s`,xpoz:k*30,ypoz:i*30,
    bcgSize:`180px 180px`
  };
  ...
  for( let i=0; i<kvadArrayShuffled.length; i++){
        ...
        if(kvadArrayShuffled[i].style.backgroundPosition==kvadOriginalArr[i].style.backgroundPosition){
         ...
         let kvadFinal=kvadArrayShuffled[i];
          ...
          kvadFinal.style.transition=('all 0.8s');
